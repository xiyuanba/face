...
person_name = st.text_input('姓名')
if st.button('为上传图片建立索引') and person_name is not None:
    # 从指定目录读取上传的文件内容
    ...
    for d in da:
        ...
        st.write("图中人物为：", d.text)

uploaded_file_search = st.file_uploader("请上传需要搜索的图片", type=["png", "jpg", "jpeg", "gif"])
if st.button('搜索') and uploaded_file_search is not None:
    # 如果已经选择了文件，则进行处理
