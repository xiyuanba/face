...
if st.button('人脸库匹配') and uploaded_file_search is not None:
    ...
    # t1 = Document(text=query_keyword)
        ...
        if len(result_list) == 0:
            ...
            for i in 1,2,3,4,5:
                # 打开图片并调整大小
                img = Image.open(result_list[i]['uri'])
                width, height = img.size
                ...
                # 在对应的列中显示图片和相关信息
                with cols[col_index]:
                    st.image(img, caption=result_list[i]['text'], use_column_width=True)
                    st.write("余弦相似度: ", result_list[i]['score'])
